...
试题库服务位于 `eleadinedu-exam` 模块。管理接口前缀为 `/backend/v1/question-bank`，学员练习接口前缀为 `/api/v1/question-bank`。列表、详情、新增、修改、删除、导入、导出和练习接口全部只接受 `POST`；其他 HTTP 方法由过滤器统一返回 HTTP 405。
...
在 `eleadinedu-api` 目录执行：

```powershell
.\mvnw.cmd -B clean -pl eleadinedu-api -am test
```
