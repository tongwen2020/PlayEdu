...
export default function ExamPaperPage() {
  ...
  return <div className={styles.page}>
    ...
    <Card>
      ...
      <Table rowKey="id" loading={loading || busy} dataSource={rows} scroll={{ x: 1330 }} pagination={{ current: query.page, pageSize: query.size, total, showSizeChanger: true, showTotal: value => `共 ${value} 份`, onChange: (page, size) => setQuery(old => ({ ...old, page, size })) }} columns={[
        ...
        { title: "版本/修订", width: 100, render: (_, paper) => `v${paper.currentVersion} / r${paper.revision}` }, { title: "题数", dataIndex: "questionCount", width: 70 }, { title: "总分", dataIndex: "totalScore", width: 80 }, { title: "通过分", dataIndex: "passScore", width: 80 },
    ...
    <Modal open={Boolean(validation)} width={680} title="发布前校验" onCancel={() => setValidation(undefined)} okText="确认发布" okButtonProps={{ disabled: !validation?.result.valid || !canPublish }} onOk={() => validation && act(async () => { await api.publish(validation.paper.id, validation.paper.revision); setValidation(undefined); }, "试卷发布成功")}>
      {validation && <><Alert showIcon type={validation.result.valid ? "success" : "error"} message={validation.result.valid ? "校验通过，可以发布" : `发现 ${validation.result.errors.length} 个问题`} /><Descriptions size="small" column={4} style={{ marginTop: 16 }}><Descriptions.Item label="题数">{validation.result.summary.questionCount}</Descriptions.Item><Descriptions.Item label="总分">{validation.result.summary.totalScore}</Descriptions.Item><Descriptions.Item label="通过分">{validation.paper.passScore}</Descriptions.Item><Descriptions.Item label="主观题分">{validation.result.summary.subjectiveScore}</Descriptions.Item></Descriptions>{validation.result.errors.map((error, i) => <Alert key={i} type="error" message={error} style={{ marginTop: 8 }} />)}{validation.result.warnings.map((warning, i) => <Alert key={i} type="warning" message={warning} style={{ marginTop: 8 }} />)}{!canPublish && validation.result.valid && <Alert type="info" message="校验已通过，但当前账号没有发布权限。" style={{ marginTop: 8 }} />}</>}
    </Modal>
    <Drawer open={Boolean(preview)} width="min(980px, 96vw)" title={preview ? `${preview.name} · ${preview.version ? `正式版本 v${preview.version}` : "当前草稿"}` : "试卷预览"} onClose={() => { setPreview(undefined); setHistory([]); }}>
      {preview && <><Space wrap style={{ marginBottom: 16 }}><Button onClick={() => void showVersion(undefined)}>当前草稿</Button>{history.map(version => <Button key={version.version} type={preview.version === version.version ? "primary" : "default"} onClick={() => void showVersion(version.version)}>v{version.version}</Button>)}</Space>
        <Card size="small" className={styles.summary}><Space size={32} wrap><Statistic title="题数" value={preview.questionCount} /><Statistic title="总分" value={preview.totalScore} precision={2} /><Statistic title="通过分数" value={preview.passScore} precision={2} /><Statistic title="客观题" value={preview.objectiveScore} precision={2} /><Statistic title="主观题" value={preview.subjectiveScore} precision={2} /></Space></Card>
        <Alert showIcon type="info" message={`通过规则：成绩达到 ${preview.passScore} 分即判定通过（试卷满分 ${preview.totalScore} 分）`} style={{ marginBottom: 16 }} />
