# eleadinedu-course 模块协作指南

## 模块职责

本模块维护课程、分类、章节、课时、附件、课程可见范围、学习记录和学习时长统计。它依赖 `eleadinedu-common` 与 `eleadinedu-resource`，不直接暴露 HTTP 接口。

当前固定试卷在线测评与考试记录不与课程绑定，归属 `eleadinedu-exam`。规划中的课程考试门槛或完成条件尚未实现，不要在课程进度服务中自行推导考试结果。
...
相关 HTTP 接口位于应用模块的课程、章节、课时、附件和用户学习 Controller。资源上传、元数据和播放地址由 `eleadinedu-resource` 负责，本模块只保存资源关联；跨领域删除由应用模块事件监听器协调。
...
- 资源文件生命周期属于 `eleadinedu-resource`；本模块保存关联，不应绕过资源服务直接操作对象存储。
