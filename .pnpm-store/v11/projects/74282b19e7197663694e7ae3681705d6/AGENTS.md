...
本文件适用于 `eleadinedu-pc/` 目录及其全部子目录。它面向在本模块中工作的 Codex/开发人员，用于说明项目结构、运行方式、实现约束和验证要求。若更深层目录后续增加 `AGENTS.md`，应同时遵守距离目标文件最近的规则。
...
Token 保存在 `localStorage` 的 `eleadinedu-frontend-token`，请求头使用 `Authorization: Bearer <token>`。当前部门 ID 和名称也保存在 `localStorage`。不要在新页面中自行实现另一套登录态或部门状态。
...
考试中心接口封装在 `src/api/exam.ts`；当前页面接线及相关后端契约如下：

| 功能 | 接口 |
| --- | --- |
| 开放题库 | `POST /api/v1/question-bank/banks/list` |
| 练习题列表 | `POST /api/v1/question-bank/questions/list` |
| 整卷提交与自动阅卷 | `POST /api/v1/question-bank/practice/paper/submit` |
| 整卷历史（后端已提供，当前页面未接入） | `POST /api/v1/question-bank/practice/paper/history` |
| 逐题提交兼容接口 | `POST /api/v1/question-bank/practice/submit` |
| 逐题历史（当前“我的作答记录”使用） | `POST /api/v1/question-bank/practice/history` |

实现和修改时遵守以下约束：

- 学员题目响应不应包含标准答案、解析或后台评分规则；提交后才能展示服务端返回的答案与解析。
- 当前自动练习支持单选、多选和判断题；页面一次加载最多 500 道启用的客观题，简答题依赖人工评分，不在该流程展示。
- 作答期间答案只保存在页面状态；提交整卷时仅发送已作答题目，服务端会把题库内未答客观题计为 0 分。
- 整卷提交使用 8～64 位请求标识实现幂等；答案集合未变化时重试必须复用原请求标识，答案变化后生成新标识。
- 服务端判分是唯一可信结果。交卷后才展示总分、正确数、逐题得分、标准答案和解析，前端不可自行计算正确与否。
- 交卷后整卷答案不可修改；离开页面前提示尚未提交的答案不会保存。
- 后端已持久化整卷答卷并提供 `practice/paper/history`，但当前 `/exam` 的“我的作答记录”仍调用旧的逐题历史接口；修改历史页时应优先完成整卷历史接线，不要把旧列表误述为新答卷记录。
...
- 改考试中心：至少覆盖题库列表、三种客观题、答题卡切换、未答交卷提示、整卷幂等提交、总分与逐题解析，以及逐题/整卷历史的实际接线状态。
...
- 学员 API Controller 位于 `eleadinedu-api/eleadinedu-api/src/main/java/xyz/eleadinedu/api/controller/frontend/`。
- 课程领域主要位于 `eleadinedu-api/eleadinedu-course/`。
- 考试题库、判分和练习记录位于 `eleadinedu-api/eleadinedu-exam/`，其 HTTP Controller 在应用模块。
