...
EleadinEdu 是企业内部培训系统，采用前后端分离架构。开源版主要提供：
...
```text
EleadinEdu/
├─ eleadinedu-api/       Java/Spring Boot 后端 Maven 多模块工程
├─ eleadinedu-admin/     React 管理后台
├─ eleadinedu-pc/        React PC 学员端
├─ eleadinedu-h5/        React H5 学员端
...
后端父工程位于 `eleadinedu-api/pom.xml`，聚合五个 Maven 模块：

| 模块 | 职责 | 主要依赖关系 |
| --- | --- | --- |
| `eleadinedu-common` | 用户、部门、管理员、权限、配置、LDAP 等通用领域模型、Mapper、Service、工具和上下文 | 基础模块 |
| `eleadinedu-resource` | 资源、资源分类、视频资源扩展信息及其服务 | 依赖 `common` |
| `eleadinedu-course` | 课程、章节、课时、附件、部门选课、学习记录和统计 | 依赖 `common`、`resource` |
| `eleadinedu-system` | 数据库迁移、初始数据、权限和配置检查、日志切面 | 依赖 `common` |
| `eleadinedu-api` | Spring Boot 启动入口、前后台 Controller、请求 DTO、拦截器、事件监听器、定时任务 | 依赖其余四个模块 |

启动类 `EleadineduApiApplication` 同时开启异步任务、声明式事务、定时任务、跨模块组件扫描和 Mapper 扫描。
...
项目支持两种数据库初始化方式：可按顺序执行 `eleadinedu-api/sql/create_tables.sql` 和 `eleadinedu-api/sql/init_data.sql` 完成表结构与默认超级管理员初始化；也可由 `eleadinedu-system` 中的 `MigrationCheck` 在应用启动时检查 `migrations` 表、执行内嵌 DDL 并记录已完成迁移，随后由其他 `CommandLineRunner` 补齐默认配置、权限和系统数据。
...
```powershell
cd eleadinedu-admin   # 或 eleadinedu-pc / eleadinedu-h5
pnpm install
...
2. Eclipse Temurin 17：Maven 离线预取依赖并打包 `eleadinedu-api.jar`；
...
2. `eleadinedu-api/pom.xml` 和各子模块 `pom.xml`；
3. `EleadineduApiApplication.java`、`application.yml`、`WebMvcConfig.java`；
...
EleadinEdu 2.2 是一个结构清晰、可通过 Docker 快速落地的单体式企业培训平台：后端按通用、资源、课程、系统和 API 分 Maven 模块组织，前端按管理端、PC 和 H5 独立交付；核心开源能力完整，但生产化时应优先补强默认安全配置、容器健康检查、共享状态方案和自动化测试。
