# eleadinedu-system 模块协作指南

## 模块职责

本模块维护数据库迁移、系统基础数据、应用配置、后台权限初始化、升级修正，以及 `@BackendPermission`、`@Log`、`@Lock` 的切面实现。

它依赖 `eleadinedu-common` 中的实体、Service 和注解，由可执行应用扫描并加载。
...
注解定义在 `eleadinedu-common`，业务模块只需依赖公共层即可声明横切语义，执行逻辑集中在本模块。

## 代码结构

```text
src/main/java/xyz/eleadinedu/system/
├── aspectj/       # 权限、日志和锁切面
├── checks/        # 启动检查与升级逻辑
├── domain/        # Migration 实体
...
```powershell
.\mvnw.cmd -pl eleadinedu-system -am test
