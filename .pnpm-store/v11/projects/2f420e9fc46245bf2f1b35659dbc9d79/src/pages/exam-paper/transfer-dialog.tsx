...
export default function TransferDialog({ categories, onClose, onSaved }: { categories: api.PaperCategory[]; onClose: () => void; onSaved: () => void }) {
  ...
  const template = () => downloadJson([{ code: "EXAM_SAMPLE_001", name: "示例试卷", description: "可先导入空草稿，再进入编辑器选题", categoryId: null, passScore: 0, tags: ["示例"], sections: [] }], "试卷导入模板.json");
  return <Modal open title="批量导入试卷" width={820} onCancel={onClose} confirmLoading={busy} okText={`确认导入 ${papers.length} 份`} okButtonProps={{ disabled: !papers.length }} onOk={async () => {
    ...
    <Alert showIcon type="info" message="仅新增草稿，每批最多 100 份。" description="支持单份导出文件、JSON 数组和 { papers: [...] }。导入会移除原试卷 ID、版本、修订和题目快照，保留通过分数及固定的试题 ID、版本与试卷分值。" />
    ...
    <Table rowKey="code" size="small" dataSource={papers} pagination={{ pageSize: 5 }} columns={[{ title: "编码", dataIndex: "code" }, { title: "名称", dataIndex: "name" }, { title: "通过分", dataIndex: "passScore" }, { title: "大题", render: (_, p) => p.sections.length }, { title: "试题", render: (_, p) => p.sections.reduce((sum, section) => sum + section.items.length, 0) }]} />
  </Modal>;
}
