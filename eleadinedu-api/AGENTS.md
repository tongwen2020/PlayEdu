...
本文件适用于 `eleadinedu-api/` 下的整个 Maven 多模块后端工程。进入子模块工作时，还必须读取并遵守该模块自己的 `AGENTS.md`；子模块规则用于补充或收紧本文件，不替代这里的全局约束。
...
默认 API 端口为 `9898`，默认数据库名为 `playedu`。主配置在 `eleadinedu-api/src/main/resources/application.yml`，数据库、连接池、JWT、并发登录和演示模式均支持环境变量覆盖。

## 模块与依赖

| 模块 | 主要职责 | 直接依赖 |
| --- | --- | --- |
| `eleadinedu-api` | 启动入口、Controller、Request、拦截器、事件监听和定时任务 | 全部业务模块 |
| `eleadinedu-common` | 用户、部门、权限、LDAP、配置、通用类型和基础设施 | 父工程公共依赖 |
| `eleadinedu-course` | 课程、章节、课时、附件、可见范围和学习进度 | `eleadinedu-common`、`eleadinedu-resource` |
| `eleadinedu-exam` | 题库、试卷版本、练习/固定试卷答卷、自动判分和考试记录 | `eleadinedu-common`、Spring JDBC |
| `eleadinedu-resource` | 素材分类、资源元数据、视频扩展信息和上传 | `eleadinedu-common` |
| `eleadinedu-system` | 数据迁移、系统初始化、权限/日志/锁切面 | `eleadinedu-common` |

依赖方向应保持为：公共层 → 资源/考试/系统领域层 → 课程领域层 → API 装配层。不要从底层模块反向引用 `eleadinedu-api`。
...
- `/backend/v1/**`：管理端接口，包含管理员、角色权限、用户部门、课程、资源、上传、LDAP、题库、试卷，以及指定学员的考试记录与快照。
- `/api/v1/**`：学员端接口，包含登录、课程、课时播放、学习进度、个人信息、开放题库练习，以及已发布固定试卷的自动测评与本人记录。
...
- 考试中心：题库与题目版本、固定试卷草稿/发布快照、导入导出、开放题库整卷练习、固定试卷自动测评、服务端判分、考试记录和答题快照。

管理端可组卷、校验和发布不可变试卷版本；PC 学员端可参加全客观题的当前发布版本，服务端幂等交卷、自动判分并写入 `exam_fixed_paper_attempts` 与统一 `exam_records`。学员本人和具备 `user-learn` 权限的管理员可查看历史答题快照。H5 仍只接入开放题库练习。

正式考试活动/场次、考生分配、考试时间与次数、断点暂存、简答题人工阅卷、成绩审核/发布尚未形成闭环，不能把当前固定试卷测评等同于完整考试组织能力。
...
├── docs/                   # 后端文档
├── sql/                    # SQL 资料
├── eleadinedu-api/            # 可执行应用与 HTTP 层
├── eleadinedu-common/         # 公共领域与基础设施
├── eleadinedu-course/         # 课程学习领域
├── eleadinedu-exam/           # 考试领域
├── eleadinedu-resource/       # 资源领域
└── eleadinedu-system/         # 启动检查与切面
```
...
- 完整表结构的唯一维护入口是 `sql/create_tables.sql`；考试模块构建时将它映射为 `db/create_tables.sql` 供启动迁移复用，不再维护模块内重复建表脚本。
- `sql/init_data.sql` 负责默认超级管理员、角色关系和工业机器人示例题库等初始化数据；结构变更与种子数据变更不要混写。
...
在 `eleadinedu-api/` 目录按改动范围选择最小充分验证：
...
.\mvnw.cmd -pl eleadinedu-course -am test
...
- API 默认端口 `9898`，配置文件位于 `eleadinedu-api/src/main/resources/application.yml`。
...
2. `eleadinedu-api/EleadineduApiApplication.java` 与 `application.yml`，确认启动和配置。
