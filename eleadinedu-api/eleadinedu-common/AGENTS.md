...
- 应用配置：`AppConfig`、配置常量和 `EleadineduConfig`。
...
- `eleadinedu-api` 使用本模块完成鉴权、上下文、响应和用户/部门管理。
- `eleadinedu-course` 复用用户、部门、分页和通用查询投影。
- `eleadinedu-resource` 复用配置、S3、分页和异常体系。
- `eleadinedu-system` 实现公共注解的切面，并初始化权限和配置。
- `eleadinedu-exam` 通过 `BackendBus` 获取后台操作者并复用异常规范。

## 依赖边界

- 不得依赖 `eleadinedu-api`、`eleadinedu-course`、`eleadinedu-resource`、`eleadinedu-exam` 或 `eleadinedu-system`。
- 领域专属实体、查询和规则不要为了方便放入公共模块。
- `@BackendPermission`、`@Log`、`@Lock` 等注解定义在这里，其切面实现位于 `eleadinedu-system`；修改契约时必须同步检查切面。
...
```powershell
.\mvnw.cmd -pl eleadinedu-common -am test
