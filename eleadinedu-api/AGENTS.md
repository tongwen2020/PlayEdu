...
| `eleadinedu-exam` | 题库、试卷、版本、逐题/整卷练习、自动判分和答卷历史 | `eleadinedu-common`、Spring JDBC |
...
- `/api/v1/**`：学员端接口，包含登录、课程、课时播放、学习进度、个人信息，以及开放题库的整卷练习与自动阅卷。
...
- 考试中心：题库与题目版本、固定试卷草稿/发布快照、导入导出、开放题库整卷练习、服务端自动判分和答卷历史。

当前固定试卷库与学员练习是两条独立链路：管理端可以组卷、校验和发布不可变试卷版本，PC/H5 则对明确开放的题库进行整卷练习。正式考试活动、考生分配、考试时间/次数、简答题人工阅卷和成绩发布尚未形成闭环，不能把“试卷已发布”描述成“学员可参加正式考试”。
