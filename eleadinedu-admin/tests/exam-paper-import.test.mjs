...
const paper = { id: 9, revision: 4, currentVersion: 2, code: "P1", name: "试卷一", description: "", categoryId: null, passScore: 4, tags: [], sections: [{ id: 8, title: "单选题", description: "", position: 9, shuffleQuestions: false, items: [{ id: 7, questionId: 11, questionVersion: 2, score: 5, position: 8, question: { stem: "快照" } }] }] };
...
test("accepts single export and strips paper, section, item and question snapshot identifiers", () => {
  ...
  assert.equal(value.passScore, 4);
...
test("requires a valid pass score that does not exceed the paper total", () => {
  for (const passScore of [undefined, -1, 4.001, 5.01]) assert.throws(() => parsePaperImport(JSON.stringify({ ...paper, passScore }), []), /通过分数/);
  assert.equal(parsePaperImport(JSON.stringify({ ...paper, passScore: 5 }), [])[0].passScore, 5);
});
