# eleadinedu-resource 模块协作指南

## 模块职责

本模块维护资源主数据、资源分类关联、视频扩展信息以及兼容 S3/MinIO 的上传能力。HTTP 协议位于 `eleadinedu-api`，课程引用关系位于 `eleadinedu-course`。

题目、试卷发布 JSON 和答题快照不依赖资源模块；考试附件或富媒体题干目前不在已实现范围内。若以后增加资源引用，必须同时定义授权访问、删除引用检查和历史快照保留策略。

它依赖 `eleadinedu-common` 的配置、S3 工具、分页与异常体系，课程模块通过资源 ID 使用本模块数据。
...
- `eleadinedu-common` 提供存储配置、上传类型、异常、分页和 S3 工具。
- `eleadinedu-course` 保存资源引用和课时语义，不负责对象存储生命周期。
- `eleadinedu-api` 负责上传协议、请求校验、权限和响应。
...
```powershell
.\mvnw.cmd -pl eleadinedu-resource -am test
