# EleadinEdu H5 学员端业务概要

## 适用范围

本文件适用于 `eleadinedu-h5/` 及其全部子目录，用于帮助开发人员快速理解移动端学员业务、代码入口和实现边界。若更深层目录后续增加 `AGENTS.md`，同时遵守距离目标文件最近的规则。

## 模块定位

`eleadinedu-h5` 是 EleadinEdu 企业培训系统面向手机浏览器和微信内置浏览器的学员端。它只调用 `/api/v1/**` 学员 API，不直接访问管理后台 API、数据库或对象存储。

当前业务闭环为：学员登录 → 选择所属部门 → 浏览被分配的课程 → 查看目录或下载附件 → 观看视频并上报学习进度 → 在最近学习和个人中心查看学习情况；学员也可以进入考试中心选择开放题库、完成客观题整卷练习、统一交卷并查看服务端自动阅卷结果。

本模块当前不包含注册、找回密码、课程搜索、正式考试组织、限时答卷、人工阅卷、评论、收藏或付费能力。考试中心目前只支持后台明确开放题库中的单选、多选、判断题整卷自动阅卷；不要用静态数据或纯前端逻辑将未接入的能力描述为可用功能。
...
Token 使用 `localStorage` 键 `eleadinedu-h5-token`，请求头格式为 `Authorization: Bearer <token>`。接口返回 401 时清理 Token 并回到登录页。登录支持普通密码和 LDAP；是否走 LDAP 由系统配置 `ldap-enabled` 决定。

### 部门上下文

一个学员可以属于多个部门，课程可见范围和个人学习统计均依赖当前部门：

- 当前部门 ID 存在 Redux，并通过 `eleadinedu-h5-depatmentKey` 持久化。
- 当前部门名称通过 `eleadinedu-h5-depatmentName` 持久化。
...
- 页面一次加载题库内最多 500 道启用的客观题；作答期间答案只保存在页面状态，可通过上一题、下一题和答题卡切换。
- 学员统一提交整卷后由服务端自动阅卷，页面展示总分、正确数、逐题得分、标准答案和解析；未答题由服务端按 0 分计算，交卷后的答案不可修改。
- 整卷提交使用 8～64 位请求标识保证幂等；答案集合未变化时因网络失败重试应复用原请求标识，答案变化后生成新标识。
- 离开页面前提示未提交答案不会保存。后端虽已提供整卷答卷历史接口，但当前“我的作答”仍读取旧的逐题历史，整卷提交不会自动出现在该旧列表中。
...
| 考试 | `POST /api/v1/question-bank/practice/paper/submit` | 幂等提交整卷并获取总分和逐题解析 |
| 考试 | `POST /api/v1/question-bank/practice/paper/history` | 分页获取整卷答卷；后端已提供，当前页面未接入 |
| 考试 | `POST /api/v1/question-bank/practice/submit` | 兼容旧逐题提交；当前练习页不使用 |
| 考试 | `POST /api/v1/question-bank/practice/history` | 分页获取旧逐题记录；当前“我的作答”使用 |
...
- 单元测试尚未接入；`../e2e/` 提供 Playwright H5 验收测试及无需账号和数据库的本地 Mock 全量回归。业务交互变更至少执行生产构建和对应的 H5 E2E，并在必要时补充真机验证。
...
- 改考试中心：验证题库/历史空态、三种客观题、答题卡、未答题计零提示、整卷提交防重复、总分与逐题解析、分页、未提交退出提示，以及逐题/整卷历史的实际接线状态。

H5 本地全量 E2E 可从仓库的 `e2e/` 目录运行：

```powershell
pnpm test:h5:local
```

后端联调入口位于 `eleadinedu-api/eleadinedu-api/src/main/java/xyz/eleadinedu/api/controller/frontend/`。修改接口契约时同步检查对应 Controller、DTO/record、PC 学员端调用方以及后端测试。
...
4. `src/store/`、`src/utils/index.ts`、`src/eleadinedu.d.ts`。
5. 目标页面及其同目录组件和样式。
